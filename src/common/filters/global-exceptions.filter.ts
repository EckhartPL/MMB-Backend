...
import { TokenErrorCodes } from 'src/utils';
import { errorMessage, getMysqlErrorMessage } from 'src/utils/message';
...
export class GlobalExceptionFilter implements ExceptionFilter {
    ...
    const { status: statusCode, ...errorData } =
      this.tryToGetErrorData(exception);
    Logger.error({
      statusCode,
      ...errorData,
  ...
  private handleUnauthorizedTokenException(
  ...
  private handleBadRequestException(error: BadRequestException): ErrorData {
    const errorResponse =
      error.getResponse() as ErrorResponseBadRequestException;
